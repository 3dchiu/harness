...
**邏輯 4 · 真人諮詢是「全卡點通用」的加速/兜底層（不是某些卡點專屬）。**
每個卡點都有兩條路：**系統自助**（慢慢堆疊累積行動）與**真人諮詢**（想更快更有效、或照步驟仍沒改善時直接預約）。
所以主表裡標「Layer 3／真人」的卡點，只是**特別依賴判斷**的；真人選項其實覆蓋全部 16 卡點。
...
| C 入口錯誤 | 換切入點、側邊繞進（靠人脈找側門） | 人脈行動骨架（引薦／主動請教）＋ 職缺勝率(①②維度)輔助 | 用人脈找側門、橫向繞進 | 補 W | ⚪ | ✅ |
| D 市場結構困難 | 評估值不值得 ＋ 窄門策略（靠人脈判斷與切入） | 人脈行動骨架（評估＋找窄門）；真人諮詢加速 | 人脈請教＋策略判斷 | 補 W | ⚪ | ✅ |

> **突破型備註**
> - **C、D 都需要「該領域人脈」才突破** → 共用「**人脈行動骨架**」（人脈引薦／主動找人請教的步驟）：C 用人脈找**側門切入**，D 用人脈**評估值不值＋找窄門**。
> - D 不再是「真人專屬」；它有自助的人脈行動路徑，真人為加速/兜底（見邏輯 4）。
> - 「人脈行動」是跨象限的行動機制，也出現在 探索A（五場對話換地圖）與 升級（信任/位置）。

### 決策型（S低×W高｜選不出，需判斷標準）
| 卡點 | 核心解法方向 | 主要掛載工具 | 行動類型 | 補/建的軸 | 狀態 | 信心 |
|------|------------|------------|---------|----------|------|------|
| A 缺乏標準 | 建立判斷標準/權重 | 決策骨架（重步驟1-2）；標準來源接價值觀儀表板 | 定義標準＋權重 | 建 S·價值觀 | 🟢 | ✅ |
| B 風險恐懼 | 設底線、可逆性、剔除風險過高 | 決策骨架（重步驟3底線＋風險過高標示） | 設底線、低代價試水 | — | 🟢 | ✅ |
| C 資訊迴避 | 用同一套標準算出來、設截止日 | 決策骨架（重步驟4算出來；停止再查） | 算出結果＋截止日 | — | 🟢 | ✅ |

> **決策型備註**：三卡點**共用同一個「決策骨架」**（4 步：定義標準→權重→底線→重算選項），只是進入時重點不同——
> A 重「定義標準＋權重」、B 重「底線＋剔除風險過高」、C 重「算出來＋設截止日」。步驟1的標準可由**價值觀儀表板**帶入。

### 升級型（S高×W高｜上不去，系統結構限制）
| 卡點 | 核心解法方向 | 主要掛載工具 | 行動類型 | 補/建的軸 | 狀態 | 信心 |
|------|------------|------------|---------|----------|------|------|
| A 責任不足 | 爭取小決策權、讓決定被看見 | 組織影響力行動骨架（重「爭取決策權」） | 爭取/交付/曝光 | 補 組織影響力 | ⚪ | ✅ |
| B 能力缺口 | 補下一層級思維模型 | 專業梳理（專業能力）＋舉證/補強 | 補能力 | 建 S·專業 | 🟢 | ✅ |
| C 系統天花板 | 確認結構性後換舞台/更大池子 | 離職診斷 ＋ 職缺勝率（內/外換舞台） | 換舞台 | 補 W | 🟢(部分) | ✅ |
| D 信任缺口 | 先建立信任 | 組織影響力行動骨架（重「建立信任」）＋人脈 | 建信任/交付 | 補 組織影響力 | ⚪ | ✅ |
| E 位置錯誤 | 進核心專案 / 橫向移動 | 組織影響力行動骨架（重「換位置」）＋職缺勝率(內轉)＋人脈 | 進核心/橫移 | 補 組織影響力 | ⚪ | ✅ |

> **升級型備註**
> - **A/D/E 共用「組織影響力行動骨架」**（同骨架、不同進入重點）：A 重爭取決策權、D 重建立信任、E 重換到核心位置。與決策骨架/價值觀探索同屬「一具多用」模式。
> - 升級型戰場＝**組織內部的影響力與能見度**（前面 W 都是外部求職市場）；診斷上 S-W 未直接測「組織內定位」，未來可補一個輕量「組織內定位」量表。
...
| 人脈行動骨架（引薦／主動請教） | 透過人脈找側門、請教、評估窄門 | 突破C、突破D、探索A、升級D/E | 補 W·人脈 | ⚪ |
| 決策骨架（4步：標準→權重→底線→重算） | 加權決策矩陣＋底線過濾；標準接價值觀儀表板 | 決策A/B/C | — | 🟢 |
| 組織影響力行動骨架 | 爭取決策權→交付→被看見→建立信任→進核心 | 升級A/D/E | 補 組織影響力 | ⚪ |
| 真人諮詢（全卡點通用層） | 加速/兜底：想更快或自助無效時 | 全部 16 卡點 | — | 既有 |
...
- **S 軸 · 知己工具**（理解自己）：價值觀探索、興趣盤點、專業梳理、三項測驗（DISC／Holland／Career Anchor）
...
四象限全部確認完成（探索4／突破4／決策3／升級5）。原 6 個待確認點均已解決：
1. ✅ 探索C/D 共用價值觀探索（讀不同訊號）。
2. ✅ 決策A＝價值觀儀表板（決策骨架步驟1標準來源）。
3. ✅ 升級B 與 突破A 共用專業梳理。
4. ✅ 決策B/C 用決策骨架；升級A/D/E 共用組織影響力行動骨架。
5. ✅ 探索B 納入興趣盤點＋三項測驗；S 軸補上性格＝第四維。
6. ✅ 離職診斷＝入口B＋6型路由器；薪資談判＝成果兌現階段。

### 剩餘待建（屬「實作」非「映射」）
- ⚪ 興趣盤點、三項測驗（DISC/Holland/Career Anchor）、組織影響力行動骨架、人脈行動骨架、產業新聞feed、AI職業說明
- 🟡 探索A 視野太窄（探索骨架，工具方向已定，內容待細化）
- 量表門檻校準（見 `measurement-model.md`）、「組織內定位」輕量量表（升級型）
